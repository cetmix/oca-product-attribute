...
class ProductAttributeValue(models.Model):
    ...
    def create(self, vals_list):
        ...
        for product_attribute_value in res:
            # Create related product variants if apply_to_products_on_create is True
            if (
                product_attribute_value.attribute_id.apply_to_products_on_create
                and not self.env.context.get("operation_from_record")
            ):
    ...
    def write(self, vals):
        ...
        if (
            ...
            and self.linked_record_ref
            and self.attribute_id.create_from_attribute_values
            and self.attribute_id.modify_from_attribute_values
            and not self.env.context.get("operation_from_record")
        ):
            linked_field = self.attribute_id.linked_field_id
            try:
                name_value = self.convert_attribute_value(vals["name"], linked_field)
                self.linked_record_ref.with_context(
                    operation_from_attribute_value=True
                ).write({linked_field.name: name_value})
            except Exception as e:
                # If conversion fails, raise a ValidationError with details
                error_message = (
                    f"Conversion failed for the field {linked_field.name}. Error: {e}"
                )
                raise ValidationError(_("Validation Error: ") + error_message) from e
        return res
